...
import { DEFAULT_MODE, normalizeMode, type Mode } from "@/lib/modes";
...
interface Props {
  ...
  /** Mode d’usage : colonnes figées en prospection. */
...
function tableHeaderLabel(isProspectionView: boolean, col: string): string {
  if (isProspectionView) return PROSPECTION_HEADER[col] ?? COL_LABELS[col] ?? col;
...
function RelevanceBadge({
  ...
  const cls = styles[flag] || "bg-white/[0.06] text-muted-foreground border-border";
  return (
    <span
...
function SegmentTags({
  ...
  if (!keys || keys.length === 0) return <span className="text-muted-foreground">—</span>;
  return (
    <div className="flex flex-wrap gap-1 max-w-[200px]">
      {keys.map((k) => (
        <span
          key={k}
          className="inline-flex rounded border border-border bg-white/[0.04] px-1.5 py-0.5 text-[10px] text-muted-foreground"
          title={k}
        >
          {labelByKey?.[k] || k}
        </span>
...
function DesktopDetailPanel({
  ...
  return (
    <div className="flex h-full w-full max-w-[320px] flex-col bg-card border-l border-border shadow-xl">
      <header className="flex shrink-0 items-start justify-between gap-2 border-b border-border px-3 py-3">
        <h2 className="pr-2 text-sm font-semibold leading-tight text-foreground">
          {row.nom || "—"}
        </h2>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto scrollbar-thin">
        <p className="border-b border-border px-3 py-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
          Coordonnées
        </p>
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Téléphone</span>
        ...
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Site web</span>
        ...
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Adresse</span>
          <div className="mt-0.5 text-sm text-foreground">{fullAddress || "—"}</div>
        </div>
        <div className="flex flex-col gap-2 border-b border-border px-3 py-2.5">
          {row.lien_annuaire ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex w-fit items-center justify-center rounded-lg border border-border bg-white/[0.06] px-3 py-2 text-xs font-medium text-foreground transition-colors hover:bg-white/[0.1]"
              onClick={(e) => e.stopPropagation()}
            >
              Voir la fiche
            </a>
        ...
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">SIREN</span>
          <div className="mt-0.5 text-sm text-foreground">{formatValue("siren", row.siren)}</div>
        </div>
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Activité</span>
          <div className="mt-0.5 text-sm text-foreground">
            {formatValue("libelle_activite", row.libelle_activite)}
          </div>
        </div>
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Forme juridique</span>
          <div className="mt-0.5 text-sm text-foreground">
            {formatValue("forme_juridique", row.forme_juridique)}
          </div>
        </div>
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Date de création</span>
          <div className="mt-0.5 text-sm text-foreground">
            {formatValue("date_creation", row.date_creation)}
          </div>
        </div>
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Effectif</span>
          <div className="mt-0.5 text-sm text-foreground">{effectifDisplay || "—"}</div>
        </div>
        <div className="border-b border-border px-3 py-2.5">
          <span className="text-[10px] uppercase text-muted-foreground">Catégorie</span>
          <div className="mt-0.5 text-sm text-foreground">
          ...
          <>
            ...
            {finRows.map(([key, label]) =>
              hasDetailValue(row[key]) ? (
                <div key={key} className="border-b border-border px-3 py-2.5">
        ...
        {hasDetailValue(row.dirigeant_nom) && (
          <>
            ...
            <div className="border-b border-border px-3 py-2.5">
              <span className="text-[10px] uppercase text-muted-foreground">Nom</span>
              <div className="mt-0.5 text-sm text-foreground">{dirigeantLine || "—"}</div>
            </div>
            <div className="border-b border-border px-3 py-2.5">
              <span className="text-[10px] uppercase text-muted-foreground">Fonction</span>
              <div className="mt-0.5 text-sm text-foreground">
            ...
            {hasDir2 && (
              <div className="border-b border-border px-3 py-2.5">
                <span className="text-[10px] uppercase text-muted-foreground">Dirigeant 2</span>
                <div className="mt-0.5 text-sm text-foreground">
...
function MobileCard({
  ...
  labelForCol: (col: string) => string;
  /** En prospection : afficher tout le détail sans clic supplémentaire. */
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 hover:border-border transition-colors">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {row.lien_annuaire && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg text-muted-foreground hover:text-foreground active:bg-white/[0.06] transition-colors"
          ...
          {row.google_maps_url && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg text-muted-foreground hover:text-blue-400 active:bg-white/[0.06] transition-colors"
      ...
      {hasSecondary && (
        <>
                  ...
                  <div key={col} className="flex justify-between items-baseline gap-2">
                    ...
                    <span className="text-xs text-muted-foreground text-right truncate">
                      {isBenchPos && row[col] ? (
                        ...
                        <a href={`tel:${row[col]}`} className="text-muted-foreground active:text-foreground">
                          {row[col]}
                        </a>
                      ) : col === "google_maps_url" && row[col] ? (
                        <a
                          href={String(row[col])}
                          target="_blank"
                          rel="noopener noreferrer"
          ...
          <button
            ...
            className="mt-2 text-[11px] text-muted-foreground hover:text-muted-foreground active:text-foreground transition-colors flex items-center gap-1"
          >
            {expanded ? (
              <>
...
export default function ResultsTable({
  ...
  const visibleCols = useMemo(() => {
    if (isProspectionView) {
      return [...PROSPECTION_TABLE_COLS];
    }
    return colData.filter(
      (c) =>
        c !== "lien_annuaire" &&
        c !== "google_maps_url" &&
      ...
      <button
        ...
        className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-medium border transition-colors ${
          signalFilter === null
            ...
            : "bg-transparent text-muted-foreground border-border hover:text-muted-foreground hover:border-border"
        }`}
      >
        Tous
      </button>
  ...
  const relevanceBar =
    hasRelevanceData && excludedCount > 0 ? (
      <div className="px-3 pt-2 pb-1 flex flex-wrap items-center justify-between gap-2 border-b border-border">
    ...
    <div className="px-3 py-2 border-b border-border">
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-muted/50 border border-border rounded-lg pl-8 pr-3 py-2 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-border transition-colors min-h-[44px]"
        />
      </div>
    </div>
  );
    ...
    <div className="border-t border-border px-4 py-2 flex items-center justify-between">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="p-2 rounded-lg text-muted-foreground hover:text-foreground active:bg-white/[0.06] disabled:opacity-30 disabled:cursor-not-allowed min-w-[44px] min-h-[44px] flex items-center justify-center"
        >
          <ChevronLeft size={15} />
        </button>
    ...
    <div className="border-t border-border px-4 py-3">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          <span className="text-foreground font-medium tabular-nums">{dataEffective.length}</span> sur{" "}
          <span className="text-foreground font-medium tabular-nums">{total}</span> résultats
        </p>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {hasGeoData && (
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-white/[0.06] px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-white/[0.1] active:bg-white/[0.15] transition-colors min-h-[44px]"
            >
              {viewMode === "map" ? (
                <>
                  <List size={13} />
          ...
          <button
            ...
            disabled={exporting || !canExport}
            className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-white/[0.06] px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-white/[0.1] active:bg-white/[0.15] disabled:opacity-50 disabled:cursor-not-allowed transition-colors min-h-[44px]"
          >
            CSV
          </button>
        </div>
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-card overflow-hidden animate-fade-in">
      ...
      <div className="px-3 pt-3 pb-3">
        <div
          className={`${RESULTS_BODY_H} flex min-h-0 flex-col overflow-hidden rounded-lg border border-border`}
        >
          {viewMode === "map" ? (
                ...
                <div className="relative min-h-0 flex-1 overflow-hidden">
                  <div className="min-h-0 h-full overflow-x-auto overflow-y-auto scrollbar-thin">
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 z-[1] bg-card shadow-[0_1px_0_0_rgba(255,255,255,0.04)]">
                        <tr className="bg-white/[0.03]">
                          {tableCols.map((col) => (
                            <th
                              ...
                              className={`px-3 py-2 text-left text-[11px] font-medium uppercase tracking-wider whitespace-nowrap select-none ${
                                col === "nom" ? "text-foreground" : "text-muted-foreground"
                              } ${
                                col === "signaux"
                                  ? "cursor-default"
                      ...
                      <tbody className="divide-y divide-white/[0.04]">
                        {pageData.length === 0 ? (
                          <tr>
                            <td
                              colSpan={tableCols.length + (showLinksColumn ? 1 : 0)}
                          ...
                          pageData.map((row, i) => (
                            <tr
                              ...
                              className={`cursor-pointer transition-colors hover:bg-muted/50 ${
                                selectedRow === row
                                  ? "border-l-2 border-l-primary/40 bg-muted/50"
                                  : ""
                              }`}
                            >
                              {tableCols.map((col) => (
                  ...
                  <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-3 pb-2 pt-3">
                    {pageData.map((row, i) => (
                      <MobileCard
                        ...
                        defaultExpanded={isProspectionView}
                      />
                    ))}
                  </div>
                )}
